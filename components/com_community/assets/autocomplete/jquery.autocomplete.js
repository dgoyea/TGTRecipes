    ...
    Autocomplete.prototype = {
        ...
        getSuggestions: function (q) {
            ...
            if (cr && $.isArray(cr.suggestions)) {
                ...
                $.get(url, me.options.params, function (txt) {
        ...
        processResponse: function (text) {
            /*jslint evil: true */
            var response, uniques, data, suggestions, img, query, i, id;
            ...
            response.data = [].concat( response.data.slice(0) ).concat( response.data.slice(0) );
            response.suggestions = [].concat( response.suggestions.slice(0) ).concat( response.suggestions.slice(0) );
            response.img = [].concat( response.img.slice(0) ).concat( response.img.slice(0) );

            uniques = [];
            data = [];
            suggestions = [];
            img = [];
            query = response.query.toLowerCase();

            for (i = 0; i < response.data.length; i++) {
                id = '' + response.data[i];
                if (uniques.indexOf(id) >= 0) continue;
                if (response.suggestions[i].toLowerCase().indexOf(query) < 0) continue;
                uniques.push(id);
                data.push(response.data[i]);
                suggestions.push(response.suggestions[i]);
                img.push(response.img[i]);
            }
        ...
        select: function (i) {
            var selectedValue, filteredValue, f;

            selectedValue = this.suggestions[i];
            if (selectedValue) {
                filteredValue = selectedValue.match('<span>(.*)<\/span>');
                if (filteredValue && filteredValue.length)
                    selectedValue = filteredValue[1];
