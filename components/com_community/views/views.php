...
class CommunityView extends JViewLegacy {
    ...
    public function showHeader($title, $icon = null, $buttons = null) {
        ...
        if (($view == 'profile' && ($task == '' || $task == 'display')) || $view == 'frontpage' || ($task == 'viewgroup')) {
            $showHead = false;
        }
    ...
    public function attachHeaders() {
        ...
        CFactory::attach('assets/stream.js', 'js');
        ...
        // Temporary CSS fixes.
        //$css = 'assets/_' . ( $debug ? 'source' : 'release' ) . '/css/bundle.css';
        //CFactory::attach($css, 'css');

        // Application configuration.
        $js = '<script>' . PHP_EOL;
        $js .= 'joms_assets_url = "' . JURI::root(true) . '/components/com_community/assets/_' . ( $debug ? 'source' : 'release' ) . '/js/";' . PHP_EOL;
        $js .= '</script>' . PHP_EOL;
        $document->addCustomTag($js);

        // Application loader.
        if ($debug) {
            $js = 'assets/_source/js/libs/lab.js';
            CFactory::attach($js, 'js');
            $js = 'assets/_source/js/loader.js';
            CFactory::attach($js, 'js');
        } else {
