    ...
    public function multiUpload() {
        ...
        switch ($type) {
            ...
            case 'bulletin': {
                    ...
                    $users = $groupModel->getMembersId($parentTable->groupid,true);
                    //var_dump($parentTable);exit;
                    $key = array_search($my->id, $users);
                    ...
                    $params->set('url', 'index.php?option=com_community&view=groups&task=viewbulletin&groupid='.$parentTable->groupid.'&bulletinid='.$parentTable->id);

                    $params->set('filename', $_file['name']);
                    $params->set('bulletin', $parentTable->title);
                    $params->set('bulletin_url', 'index.php?option=com_community&view=groups&task=viewbulletin&groupid='.$parentTable->groupid.'&bulletinid='.$parentTable->id);
                    CNotificationLibrary::add('groups_bulletin_newfile', $my->id, $users, JText::sprintf('COM_COMMUNITY_GROUP_BULLETIN_NEW_FILE_SUBJECT'), '', 'groups.bulletin.newfile', $params);
        ...
        switch ($extension) {
            ...
            default: {
                    ...
                    if (in_array($extension, $defaultextension, true)) {
                        if ($limitstart == 0) {
                            $objResponse->addAssign('files-' . $extension . '-list', 'innerHTML', $html);
                        } else {
                            $objResponse->addScriptCall('joms.file.updateFileList', $html, 'files-' . $extension . '-list');
                        }
                    } else {
                        $objResponse->addScriptCall('joms.file.searchFileUpdate', $html);
                    }
