...
class CommunityModelUsers extends JModelLegacy {
    ...
    public function getGenderInfo() {
        ...
        $query = 'SELECT SUM(CASE ' . $db->quoteName('value') . ' WHEN ' . $db->Quote('Male') . ' THEN 1 ELSE 0 END OR CASE ' . $db->quoteName('value') . ' WHEN ' . $db->Quote('COM_COMMUNITY_MALE') . ' THEN 1 ELSE 0 END) as Male, ' .
                'SUM(CASE ' . $db->quoteName('value') . ' WHEN ' . $db->Quote('Female') . ' THEN 1 ELSE 0 END OR CASE ' . $db->quoteName('value') . ' WHEN ' . $db->Quote('COM_COMMUNITY_FEMALE') . ' THEN 1 ELSE 0 END) as Female ' .
